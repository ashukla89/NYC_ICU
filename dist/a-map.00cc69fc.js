...
},{}],"data/nyc.geojson":[function(require,module,exports) {
module.exports = "/nyc.0b7c162a.geojson";
},{}],"data/nyc_icu.csv":[function(require,module,exports) {
module.exports = "/nyc_icu.9de320ea.csv";
},{}],"scripts/nyc_icu/a-map.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#chart-a').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // center the map projection on NYC as per: http://bl.ocks.org/phil-pedruco/6646844

var projection = d3.geoMercator().center([-73.94, 40.70]).scale(50000).translate([width / 2, height / 2]);
var path = d3.geoPath().projection(projection);
var colorScale = d3.scaleSequential(d3.interpolateRdYlBu); //.clamp(true)

var radiusScale = d3.scaleSqrt().range([0, 1]);
Promise.all([d3.json(require('/data/nyc.geojson')), d3.csv(require('/data/nyc_icu.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  console.log(json); // console.log(datapoints)

  var boroughs = json.features;
  var latestData = datapoints.filter(function (d) {
  ...
  svg.selectAll('path').data(boroughs).enter().append('path').attr('class', 'borough').attr('d', path).attr('fill', 'lightgrey').attr('stroke', 'grey'); // add one circle for every hospital

  svg.selectAll('circle').data(latestData).enter().append('circle').attr('r', function (d) {
    return radiusScale(+d.total_icu_beds_7_day_avg);
  }).attr('opacity', 0.7).attr('fill', function (d) {
    return colorScale(1 - d.icu_beds_used_pct_7_day_avg);
  }) // invert the color scheme by subtracting
...
},{"d3":"../node_modules/d3/index.js","geojson":"../node_modules/geojson/geojson.js","/data/nyc.geojson":"data/nyc.geojson","/data/nyc_icu.csv":"data/nyc_icu.csv"}]},{},["scripts/nyc_icu/a-map.js"], null)
//# sourceMappingURL=/a-map.00cc69fc.js.map