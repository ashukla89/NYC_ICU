...
var svg = d3.select('#chart-c').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
...
var line = d3.line().x(function (d) {
  ...
  return yPositionScale(d.capacity);
});
d3.csv(require('/data/by_borough.csv')).then(ready).catch(function (err) {
  console.log(err);
});
...
function ready(datapoints) {
  ...
  var capacity = datapoints.map(function (d) {
    return +d.icu_beds_used_pct_7_day_avg;
  });
  console.log('dates', dates);
  xPositionScale.domain(d3.extent(dates));
  yPositionScale.domain([0, d3.max(capacity)]);
  var nested = d3.nest().key(function (d) {
    return d.Borough;
  }).entries(datapoints); // nested.push(nested[0])
  ...
  //   .attr('class', 'title')
  // const rectWidth =
  //   xPositionScale(parseTime('March-21')) -
  //   xPositionScale(parseTime('December-20'))

  var final = nested[0].values[0].icu_beds_used_pct_7_day_avg;
  console.log('final capacity', final);
  svg.selectAll('.lines').data(nested).enter().append('path').attr('class', function (d) {
  ...
  }).attr('stroke-width', 2).attr('fill', 'none').style('visibility', 'hidden');
  svg.selectAll('circle').data(nested).enter().append('circle').attr('class', function (d) {
    return 'circles ' + d.key.toLowerCase().replace(/[^a-z]/g, '');
  }).classed('expensive', function (d) {
    if (d.values[0].icu_beds_used_pct_7_day_avg > final) {
      return true;
    }
  }).attr('fill', function (d) {
    return colorScale(d.key.toLowerCase().replace(/[^a-z]/g, ''));
  }).attr('r', 4).attr('cy', function (d) {
    return yPositionScale(d.values[0].icu_beds_used_pct_7_day_avg);
  }).attr('cx', function (d) {
    return xPositionScale(d.values[0].datetime);
  }).style('visibility', 'hidden');
  svg.selectAll('.labels').data(nested).enter().append('text').attr('class', function (d) {
    // console.log(d.key)
    return d.key.toLowerCase().replace(/[^a-z]/g, '');
  }).classed('labels', true).classed('expensive', function (d) {
    if (d.values[0].icu_beds_used_pct_7_day_avg > final) {
      return true;
    }
  }).attr('y', function (d) {
    ...
    return d.key;
  }).attr('dx', 6).attr('dy', 4).attr('font-size', '12').style('visibility', 'hidden'); // here's the scrollytelling
  // console.log(nested)
  ...
  var xAxis = d3.axisBottom(xPositionScale).tickFormat(d3.timeFormat('%y-%b-%d')).ticks(9);
  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis);
  var yAxis = d3.axisLeft(yPositionScale);
  svg.append('g').attr('class', 'axis y-axis').call(yAxis); // function render() {
...
},{"d3":"../node_modules/d3/index.js","/data/by_borough.csv":"data/by_borough.csv"}]},{},["scripts/nyc_icu/c-map.js"], null)
//# sourceMappingURL=/c-map.9d6d565a.js.map