...
},{}],"scripts/nyc_icu/01-map.js":[function(require,module,exports) {
"use strict";
...
var svg = d3.select('#chart-1').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).style('background-color', 'black').append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var projection = d3.geoMercator();
var graticule = d3.geoGraticule();
var path = d3.geoPath().projection(projection);
var colorScale = d3.scaleSequential(d3.interpolateCool).clamp(true);
...
function ready(_ref) {
  ...
  var countries = topojson.feature(json, json.objects.countries); // const popExtent = d3.extent(datapoints, d => +d.population)
  ...
  svg.append('path').datum(graticule()).attr('d', path).attr('stroke', 'grey').attr('stroke-width', 0.5).attr('fill', 'none').lower(); // add one circle for every world city

  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', 0.5) // .attr('opacity', 1)
  .attr('fill', function (d) {
    return colorScale(+d.population);
  }).attr('transform', function (d) {
    var coords = [d.lng, d.lat]; // console.log(projection(coords))

    return "translate(".concat(projection(coords), ")");
  });
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/world.topojson":"data/world.topojson","/data/world-cities.csv":"data/world-cities.csv"}]},{},["scripts/nyc_icu/01-map.js"], null)
//# sourceMappingURL=/01-map.04e3e511.js.map