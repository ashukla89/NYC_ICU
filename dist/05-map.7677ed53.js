...
},{}],"scripts/nyc_icu/05-map.js":[function(require,module,exports) {
"use strict";
...
var pointScale = d3.scalePoint().range([120, height - 120]);
...
function ready(_ref) {
  ...
  radiusScale.domain(mwExt); // add one circle for every powerplant

  svg.selectAll('.plant-circle').data(datapoints).enter().append('circle').attr('r', function (d) {
  ...
  svg.selectAll('.state-text').data(states.features).enter().append('text').text(function (d) {
    return d.properties.abbrev;
  }).attr('transform', function (d) {
    return "translate(".concat(path.centroid(d), ")");
  }).attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').style('text-shadow', '-1px -1px 0 #fff, 1px -1px 0 #fff, -1px 1px 0 #fff, 1px 1px 0 #fff');
  var sources = colorScale.domain();
  pointScale.domain(sources);
  svg.selectAll('g').data(sources).enter().append('g').attr('transform', function (d) {
    return "translate(".concat(40 - margin.left, ",").concat(pointScale(d), ")");
  }).each(function (d) {
    var svg = d3.select(this);
    svg.append('circle').attr('class', function (d) {
    ...
    }).attr('dx', 12).attr('font-size', 12).attr('text-anchor', 'left').style('alignment-baseline', 'middle');
  });
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","/data/us_states.topojson":"data/us_states.topojson","/data/powerplants.csv":"data/powerplants.csv"}]},{},["scripts/nyc_icu/05-map.js"], null)
//# sourceMappingURL=/05-map.7677ed53.js.map